	...
	<div class="wrapper">
		...
		<form class="consultation__form" action="<?= POST_FORM_ACTION_URI ?>" method="POST" novalidate="novalidate">
			...
			<div class="consultation__form-wrapper">
				...
				<div class="consultation__form-wrapper-item">
					<div class="consultation__input-wrapper">
						...
						<input id="form-name" class="consultation__form-input" type="text" <? if (empty($arResult["REQUIRED_FIELDS"]) || in_array("NAME", $arResult["REQUIRED_FIELDS"])) : ?>required="" data-validate-field="name" <? endif ?> name="user_name" value="<?= $arResult["user_name"] ?>" placeholder="<?= GetMessage("MFT_NAME") ?>">
						<label for="form-name" class="consultation__label-name">
				...
				<div class="consultation__form-wrapper-item">
					<div class="consultation__textarea-wrapper">
						...
						<textarea id="form-comm" name="MESSAGE" placeholder="<?= GetMessage("MFT_MESSAGE") ?>" class="consultation__form-input consultation__form-textarea" <? if (empty($arResult["REQUIRED_FIELDS"]) || in_array("MESSAGE", $arResult["REQUIRED_FIELDS"])) : ?>data-validate-field="MESSAGE" required="" <? endif ?>><?= $arResult["MESSAGE"] ?></textarea>
						<label for="form-comm" class="consultation__label-textarea">
							<svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
			...
			<div class="consultation__buttons">
				...
				<? if ($arParams["PRICELIST_FILE"]) : ?>
					<div class="consultation__btn-download-wrapper">
						<a class="consultation__btn-download" href="<?= CFile::GetPath($arParams["PRICELIST_FILE"]); ?>" download><?= GetMessage("PRICELIST"); ?></a>
					</div>
				<? endif; ?>
			</div>
		</form>
