...
(function (exports,ui_vue3,ui_vue3_pinia) {
    ...
    */
   var HlList = ui_vue3.BitrixVue.mutableComponent('HlList', {
     name: 'HlList',
     props: ['arParams'],
     emits: ["pagerFunc"],
     data: function data() {
     ...
     computed: _objectSpread({}, ui_vue3_pinia.mapStores(dropListHeaderStore)),
     methods: {
       GetList: function GetList(currentPage) {
         var _this = this;
         var count = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : this.CountPageDef;
       ...
       // переключение кол-ва
       CountPage: function CountPage(res) {
         setPreloader($(".notifications-news__list"));
         this.CountPageDef = res.data.count;
         this.GetList(res.data.pageN, res.data.count);
       },
       // изменение просмотренности элемента
       updateView: function updateView(res) {
         var _this2 = this;
         var result = babelHelpers.asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee() {
           return _regeneratorRuntime().wrap(function _callee$(_context) {
             while (1) switch (_context.prev = _context.next) {
               case 0:
                 _context.next = 2;
                 return BX.ajax.runComponentAction("itin:hl.news.list", "update", {
                   mode: "class",
                   data: {
                     hl_id: _this2.arParams.HL_ID,
                     ids: [res.data.id]
                   }
                 }).then(function (response) {
                   if (response.data.result) {
                     // Получаем непрочитанные сообщения из хранилища
                     var units = _this2.dropListHeaderStoreStore.getUnit(_this2.arParams.HL_ID);
       ...
       updateAll: function updateAll() {
         var _this3 = this;
         BX.ajax.runComponentAction("itin:hl.news.list", "update", {
           mode: "class",
           data: {
             hl_id: this.arParams.HL_ID
           }
     ...
     mounted: function mounted() {
       var _this4 = this;
       // Получаем непрочитанные сообщения из хранилища
       var units = this.dropListHeaderStoreStore.getUnit(this.arParams.HL_ID);
       // Если нет, скрываем кнопку
       if (!units.list.length) {
         this.showBtn = false;
       }
       ...
       var pagen = urlParams.searchParams.get('PAGEN_1');
       var count = urlParams.searchParams.get('count');
       ...
       BX.ajax.runComponentAction("itin:hl.news.list", "getList", {
         ...
         Object.assign(_this4.items, response.data.result);
...
//# sourceMappingURL=hl-list.bundle.js.map
