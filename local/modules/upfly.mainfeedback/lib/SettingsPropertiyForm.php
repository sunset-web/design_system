...
class SettingsPropertiyForm
{
   ...
   public static function GetSettingsHTML($arProperty = false, $strHTMLControlName = false, &$arPropertyFields = false)
   {
      ...
      $hidden = false;
      if (!empty($arProperty["USER_TYPE_SETTINGS"]["HIDDEN"]))
         $hidden = true;

      $phone = false;
      if (!empty($arProperty["USER_TYPE_SETTINGS"]["PHONE"]))
         $phone = true;

      $mail = false;
      if (!empty($arProperty["USER_TYPE_SETTINGS"]["MAIL"]))
         $mail = true;
