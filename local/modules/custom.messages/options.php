...
<form method="POST" action="<?= $APPLICATION->GetCurPage() ?>?mid=<?= htmlspecialcharsbx($request['mid']) ?>&amp;lang=<?= $request['lang'] ?>" name="<?= strtolower($arModConf['name']) ?>_settings" enctype="multipart/form-data">
    <? foreach ($aTabs as $aTab) : ?>
        <? if ($aTab['OPTIONS']) : ?>
